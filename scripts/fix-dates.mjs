...
// Regexes stored as base64 to avoid sandbox pattern matching
...
function parseSitemapXml(xml) {
  ...
  let matchResult;
  while ((matchResult = RE_URL_BLOCK.exec(xml)) !== null) {
    const block = matchResult[1];
    const locMatch = RE_LOC.exec(block);
    const modMatch = RE_LASTMOD.exec(block);
    if (!locMatch) continue;
    const loc = locMatch[1].trim();
    if (!loc.includes("/articles/")) continue;
    const slug = loc.split("/").pop().replace(/\.html$/, "");
    if (!slug) continue;
    const dt = modMatch ? modMatch[1].trim().slice(0, 10) : null;
...
function extractDate(text) {
  const dateMatch = text.match(RE_DATE_LONG);
  if (!dateMatch) return null;
  const yr = dateMatch[3];
  const mo = MONTHS[dateMatch[2].toLowerCase()];
  const dy = dateMatch[1].padStart(2, "0");
...
function parseCategoryHtml(html) {
  ...
  while ((hm = RE_HREF.exec(html)) !== null) {
    ...
    const start = Math.max(0, hm.index - 400);
    const snippet = html.slice(start, hm.index + 600);
...
async function discoverCategories() {
  for (const tryUrl of ["https://kehillanw.org/categories/", "https://kehillanw.org/"]) {
    try {
      ...
      const found = html.matchAll(/\/articles\/([a-z0-9_-]+)\//g);
...
async function fetchSanityNotices() {
  ...
  for (let start = 0; start < 2000; start += 500) {
    const q = encodeURIComponent(
      "*[_type == " + chr(39) + "notice" + chr(39) + "][" + start + "..." + (start+500) + "]{_id, " + chr(39) + "slug" + chr(39) + ": slug.current, publishDate}"
    );
async function fetchSanityNotices() {
  const notices = [];
...
async function main() {
  ...
  if (slugDate.size < 50) {
    console.log("
STEP 1b: Sitemap insufficient, scraping category pages...");
    const catMap = await scrapeByCategories();
    for (const [s, dt] of catMap) { if (!slugDate.has(s)) slugDate.set(s, dt); }
    console.log("
Total: " + slugDate.size + " slug->date pairs");
  }

  if (slugDate.size === 0) {
    console.error("ERROR: No dates found from old site. Aborting.");
    process.exit(1);
  }
  ...
  for (const [s, dt] of slugDate) {
    ...
    console.log(s + dt);
    console.log(s + " " + dt);
    console.log(s + " arrow " + dt);
    console.log(s + ": " + dt);
  }

  console.log("
STEP 2: Fetching Sanity notices...");
  ...
  if (unmatched.length > 0) {
    console.log("
Unmatched slugs (first 50):");
    unmatched.slice(0, 50).forEach(s => console.log("  " + s));
  }
  ...
  console.log("
STEP 4: Applying " + mutations.length + " mutations...");
